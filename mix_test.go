...
var addr_arithm_by_compile_asm string

//go:embed testdata/addr_arithm_by_compile-mixed.asm
var addr_arithm_by_compile_mixed_asm string

func TestMix_addr_arithm_by_compile(t *testing.T) {
	require := require.New(t)

	os.Chdir("testdata")
	var r = bufio.NewReader(strings.NewReader(addr_arithm_by_compile_asm))
	var w strings.Builder
	Mix(r, &w)

	require.Equal(addr_arithm_by_compile_mixed_asm, w.String())
}

//go:embed testdata/strconv.Atoi.asm
var strconv_Atoi_asm string

//go:embed testdata/strconv.Atoi-mixed.asm
var strconv_Atoi_mixed_asm string

func TestMix_strconv_Atoi(t *testing.T) {
	require := require.New(t)

	os.Chdir("testdata")
	var r = bufio.NewReader(strings.NewReader(strconv_Atoi_asm))
	var w strings.Builder
	Mix(r, &w)

	require.Equal(strconv_Atoi_mixed_asm, w.String())
}
